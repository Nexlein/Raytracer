...
//     Ray localRay(Point3D(ray._origin._x - _position._x,
//                          ray._origin._y - _position._y,
//                          ray._origin._z - _position._z), ray._direction);
...
//     ro = rotateX(ro, rx); ro = rotateY(ro, ry); ro = rotateZ(ro, rz);

//     Point3D rd = { localRay._direction._x, localRay._direction._y, localRay._direction._z };
//     rd = rotateX(rd, rx); rd = rotateY(rd, ry); rd = rotateZ(rd, rz);
...
//                     n = rotateX(n, fx); n = rotateY(n, fy); n = rotateZ(n, fz);
...
bool RayTracer::PrimitiveGroup::hits(const Ray& ray, HitRecord& hitRecord) const
{
    ...
    for (auto& child : _children) {
        if (child->hits(rotatedRay, tempRec)) {
            if (tempRec.distance < closest && tempRec.distance > 0.001) {
                closest = tempRec.distance;
                hitRecord = tempRec;
